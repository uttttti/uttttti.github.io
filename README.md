# I'm a Frontend Engineer && Web Director.
...
-  2020/9 ~ 
    ...
    | 派遣社員で、React+ReduxToolkit+MaterialUIを使用したキャンペーンサイト、Vue+ElementUIを使用した管理CMS、ReactとReactNativeを使用したPoC、脆弱性対応など多数の案件を担当しました。特にPoCの案件では学習の時間を多く設けていただけ他ので、Reactの最新仕様や、AmplifyなどのAWSの機能にも触れることができ、大変良い経験をさせていただきました。 | 
...
-  2019/2 – 2020/6 
    ...
    | ----- |
    | フロントエンドエンジニア |
    | 派遣社員で、1年ほどVue+Vuex+VuetifyでB2CのWebサービスを、その後、半年ほどAngular+Angular MaterialでB2BのWebサービスを開発していました。フロントエンドエンジニアとしてだけでなく、簡単なUIパーツをデザインしたり、画面のレイアウトを調整したりなど、少しだけWebデザイナーの領域のお手伝いもしました。 | 
...
-  2011/5 – 2018/9 
    ...
    | 入社時は広告事業の部署でした。そこで2年ほどFlashでモバイル広告バナーを作成しつつ、入稿管理や代理店との調整を行いました。その後、メディア制作事業の部署に異動し、主にチームリーダーとして案件管理や進行管理、メンバーのマネジメントを4年間ほどした後、フロントエンドエンジニアにスキルチェンジして、Webサービスやハイブリッドアプリの実装を担当しました。 | 
...
GoogleMapsAPIを使った実店舗と連動したキャンペーンで、デザイン、フロントエンド開発を1名で担当しました。最初のリリースが実質2週間という短納期の案件でしたが、メンバー同士の適切なコミュニケーションやフォローもあって無事にリリースすることができました。

`React` `Redux Toolkit` `Typescript` `Material UI` `Backlog` `Git` `BacklogGit` `Slack` `GoogleMapsAPI`
...
`HTML` `CSS` `Javascript` `Typescript` `React` `Vue.js` `Angular` `Vuex` `Redux` `RxJS` `Vuetify` `Angular Material` `Material-ui` `ElementUI` `Backbone.js` `SCSS` `Stylus` `EJS` `Jade` `Storybook` `npm-scripts` `gulp` `webpack` `Babel` `Github` `Bitbucket` `Codecommit` `BacklogGit` `Ionic` `React Native` `jQuery` `ActionScript` `smarty` `twig` `Illustrator` `Photoshop` `Sketch` `InVision` `Figma` `Agile` `Google Analytics` `Google Optimize` `Amplify`
